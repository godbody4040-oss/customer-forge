...
const SetupReminderEmail = (p: LifecycleProps) => (
  ...
  >
    ...
    <Text style={text}>{p.nextActionWhy || 'It takes a couple of minutes and unlocks the rest.'}</Text>
    {typeof p.daysLeft === 'number' && p.daysLeft > 0 ? (
      <Text style={text}>
        You have <strong>{p.daysLeft} day{p.daysLeft === 1 ? '' : 's'}</strong> of free full access
        left{p.trialEnds ? ` (ends ${p.trialEnds})` : ''}.
      </Text>
    ) : null}
...
const BookingFollowUpEmail = (p: LifecycleProps) => (
  ...
  >
    <Text style={text}>
      {p.bookingName || 'A customer'} requested{p.bookingWhen ? ` ${p.bookingWhen}` : ' an appointment'}.
      It's already in your calendar and CRM.
    </Text>
    <Text style={text}>
...
const WinbackEmail = (p: LifecycleProps) => (
  ...
  >
    <Text style={text}>
      It's been {p.daysAway || 7} days since you were in {p.businessName || 'your workspace'}. Nothing
      was lost — your site, services, forms and settings are exactly as you left them.
    </Text>
    {p.leadCount && p.leadCount > 0 ? (
      <Text style={text}>
        You have <strong>{p.leadCount}</strong> lead{p.leadCount === 1 ? '' : 's'} waiting for a reply.
      </Text>
    ) : null}
    <Text style={text}>
